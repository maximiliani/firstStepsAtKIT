...
//    }
    ...
    public boolean isValid(String input) throws ValidationException {
        ...
        if (matcher.find()) {
            ...
            if (input[0].length() == matcher.group(0).length() && matcher.group(0).length()<255 && domainValidator.isValid(new String[]{matcher.group(2)})) return true;
        }
        throw new ValidationException("Invalid mail address", new IllegalArgumentException());
    }
